// =========================================================================
// STOU 10151 Thai Studies Web App - Application Logic
// =========================================================================
...
(function() {
  ...
  const state = {
    ...
    theme: localStorage.getItem('stou_10151_theme') || 'light',
    ...
    practice: {
      ...
      bookmarked: new Set(JSON.parse(localStorage.getItem('stou_10151_bookmarks') || '[]')),
      wrongQuestions: new Set(JSON.parse(localStorage.getItem('stou_10151_wrong') || '[]'))
    },
  ...
  function applyTheme(theme) {
    ...
    localStorage.setItem('stou_10151_theme', theme);
  ...
  function updateDashboardStats() {
    ...
    const savedResult = localStorage.getItem('stou_10151_last_result');
  ...
  function checkExistingExamSession() {
    const savedSession = localStorage.getItem('stou_10151_active_exam');
    if (savedSession) {
      try {
        ...
        console.error('Failed to restore 10151 exam session', e);
      }
    }
  }
  ...
  function saveExamSession() {
    ...
    localStorage.setItem('stou_10151_active_exam', JSON.stringify(data));
  }
  ...
  function submitExam() {
    ...
    localStorage.removeItem('stou_10151_active_exam');
    ...
    localStorage.setItem('stou_10151_wrong', JSON.stringify(Array.from(state.practice.wrongQuestions)));
    ...
    localStorage.setItem('stou_10151_last_result', JSON.stringify(state.exam.result));
  ...
  function renderResults() {
    ...
    if (elements.resultsHero) {
      ...
      const gradeDesc = res.grade === 'H' ? '🎉 ยอดเยี่ยมมาก! คุณผ่านการประเมินในระดับเกียรตินิยม (Honor) มีความเข้าใจความเป็นไทยในทุกมิติอย่างลึกซึ้ง' :
                        res.grade === 'S' ? '✅ ยินดีด้วย! คุณผ่านเกณฑ์การประเมินวิชาไทยศึกษาตามมาตรฐาน มสธ. (60% ขึ้นไป) ทบทวนหน่วยที่ยังพร่องอีกนิดจะสอบได้ H สบายๆ' :
                        '⚠️ คะแนนยังไม่ผ่านเกณฑ์ 60% ของ มสธ. แนะนำให้อ่านสรุปเนื้อหาในหน่วยที่มีคะแนนต่ำ และฝึกทำโจทย์ซ้ำอีกรอบ';
    ...
    QUESTIONS_DATA.forEach((q, idx) => {
      ...
      card.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem;">
          <span style="font-family: var(--font-heading); font-weight: 600; color: var(--primary-gold-dark);">
        ...
        <div class="explanation-box">
          <div class="explanation-header">📖 คำอธิบายเฉลยและบริบทความรู้</div>
  ...
  function toggleBookmarkCurrentPractice() {
    ...
    localStorage.setItem('stou_10151_bookmarks', JSON.stringify(Array.from(state.practice.bookmarked)));
