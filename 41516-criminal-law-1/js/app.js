/**
 * STOU 41516 Criminal Law 1: Application Controller
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const state = {
    theme: localStorage.getItem('stou41516_theme') || 'light',
    currentTab: 'overview',
    ...
    essay: {
      currentIndex: 0,
      drafts: JSON.parse(localStorage.getItem('stou41516_essay_drafts') || '{}'),
      rubricScores: JSON.parse(localStorage.getItem('stou41516_essay_rubrics') || '{}'),
      revealedAnswers: {}
    },
    ...
    practice: {
      ...
      revealed: {},
      userAnswers: {},
      bookmarked: new Set(JSON.parse(localStorage.getItem('stou41516_bookmarks') || '[]'))
    },
  ...
  function initTheme() {
    ...
    if (themeBtn) {
      themeBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('stou41516_theme', state.theme);
        document.documentElement.setAttribute('data-theme', state.theme);
        updateThemeIcon();
      });
    }
  ...
  function renderResults(correctCount, unitBreakdown) {
    ...
    if (percentage >= 76) {
      ...
      gradeDesc = 'ยอดเยี่ยมมาก! คุณมีความรู้ความเข้าใจหลักกฎหมายอาญา 1 ในเกณฑ์สูงมาก พร้อมสำหรับสนามสอบจริง';
  ...
  function renderEssay() {
    ...
    if (textarea && wordCount) {
      ...
      textarea.addEventListener('input', () => {
        ...
        localStorage.setItem('stou41516_essay_drafts', JSON.stringify(state.essay.drafts));
      });
    }
    ...
    const updateRubricScore = () => {
      ...
      localStorage.setItem('stou41516_essay_rubrics', JSON.stringify(state.essay.rubricScores));
  ...
  window.toggleBookmark = function(qid) {
    ...
    localStorage.setItem('stou41516_bookmarks', JSON.stringify(Array.from(state.practice.bookmarked)));
    renderPractice();
  };
